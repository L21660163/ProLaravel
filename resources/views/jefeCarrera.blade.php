<x-app-layout>
    ...
    <nav class="bg-gray-800 text-white py-3 px-6">
        <ul class="flex space-x-6">
            <li>
                <button 
                    onclick="showPendingProjects()" 
                    class="px-4 py-2 bg-gray-700 rounded hover:bg-gray-600 transition-all">
                    Pendientes
                </button>
            </li>
            <li>
    ...
    <script>
        ...
        function updateProjectPhaseReject(projectId) {
            fetch(`/projects/update-phase/${projectId}`, {
                ...
                body: JSON.stringify({
                    id_project_phase: 1 // Cambia la fase a 1
                })
            })
            .then(response => response.json())
            .then(data => {
        ...
        // Función para mostrar proyectos pendientes
        function showPendingProjects() {
            alert('Mostrando proyectos pendientes (implementación futura).');
        }

        // Función para asignar revisor
        function assignReviewer() {
            alert('Asignar revisor (implementación futura).');
        }

        // Función para asignar asesor
        function assignAdvisor() {
            alert('Asignar asesor (implementación futura).');
        }
    </script>
</x-app-layout>